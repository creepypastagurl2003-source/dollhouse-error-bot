...
def _get_ambient_channel(guild: discord.Guild, cfg: dict) -> discord.TextChannel | None:
...
class Core(commands.Cog):
    ...
    async def help_cmd(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            ...
            value=(
                ...
                "`/testambient` — Send a test ambient message now\n"
    ...
    @app_commands.command(name="setchannel", description="Set the channel for ambient FNAF messages.")
    ...
    async def setchannel(self, interaction: discord.Interaction, channel: discord.TextChannel = None):
        ...
        cfg = _load_cfg()
        cfg[str(interaction.guild.id)] = target.id
        _save_cfg(cfg)
        await interaction.response.send_message(
            f"📷 *Camera feeds redirected to {target.mention}. They'll know where to look.* 👁️",
            ephemeral=True,
        )
    ...
    async def testambient(self, interaction: discord.Interaction):
        cfg = _load_cfg()
        channel = _get_ambient_channel(interaction.guild, cfg)
        if channel:
            messages = sys.modules["__main__"].MOVEMENT_MESSAGES
